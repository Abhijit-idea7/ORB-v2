...
main.py
-------
ORB v2 — Dual-Window Opening Range Breakout Trading Bot.

Two ORB windows run in every loop tick per stock:
  Primary   (15-min range): entries 9:30–11:00 IST
  Secondary (30-min range): entries 9:45–11:30 IST

Primary is checked first. Secondary fires only when primary produces no signal.
ONE_TRADE_PER_STOCK_PER_DAY ensures each stock trades at most once.
...
Lifecycle (GitHub Actions):
  ...
  4. Loop every 2 minutes:
       ...
       c. Scan candidates for new entry signals (both ORB windows)
...
def run() -> None:
    logger.info("=" * 65)
    logger.info("ORB v2 — Dual-Window Opening Range Breakout Bot")
    ...
    logger.info(f"Windows     : 15-min ORB (→11:00) + 30-min ORB (→11:30)")
